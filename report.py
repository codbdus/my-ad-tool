...
st.warning(f"📢 [시스템 진단] 인식된 컬럼명 목록: {list(df.columns)}")
...
def to_num(s):
    if s is None or s not in df.columns: return pd.Series(0, index=df.index)
    return pd.to_numeric(df[s].astype(str).str.replace(r'[^\d]', '', regex=True), errors='coerce').fillna(0)

t_imp = int(to_num(f_imp).sum())
t_clk = int(to_num(f_clk).sum())
t_cost = int(to_num(f_
